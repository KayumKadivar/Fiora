  ...
  return (
    <div className="w-full max-w-md p-6 sm:p-8 rounded-3xl bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-zinc-200 dark:border-white/10 shadow-2xl relative overflow-hidden transition-all duration-500">
      ...
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-xs sm:text-md transition-colors">{subtitle}</p>
      </div>

      <form onSubmit={handleSubmit} noValidate className="space-y-6 z-10 relative">
        <div className="space-y-2">
          <label className="text-md font-medium text-zinc-700 dark:text-zinc-300 block text-left transition-colors">Email Address</label>
          <div className="relative">
            ...
            <input
              ...
              className={`block w-full pl-11 pr-4 py-3.5 border ${errors.email ? 'border-rose-500' : 'border-zinc-200 dark:border-white/10'} rounded-2xl bg-white dark:bg-black/20 text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 shadow-sm`}
              placeholder={role === 'admin' ? "admin@fiora.com" : "user@example.com"}
            />
          </div>
          {errors.email && <p className="text-rose-500 text-[14px] font-bold mt-1 ml-1 animate-in fade-in slide-in-from-top-1">{errors.email}</p>}
        </div>

        <div className="space-y-2">
          <label className="text-md font-medium text-zinc-700 dark:text-zinc-300 block text-left transition-colors">Password</label>
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <input
              ...
              className={`block w-full pl-11 pr-4 py-3.5 border ${errors.password ? 'border-rose-500' : 'border-zinc-200 dark:border-white/10'} rounded-2xl bg-white dark:bg-black/20 text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 shadow-sm`}
              placeholder="••••••••"
            />
          </div>
          {errors.password && <p className="text-rose-500 text-[14px] font-bold mt-1 ml-1 animate-in fade-in slide-in-from-top-1">{errors.password}</p>}
        </div>

        <button
          type="submit"
          className={`w-full flex items-center justify-center py-3.5 px-4 border border-transparent rounded-2xl shadow-lg text-md font-semibold text-white transition-all duration-300 transform hover:scale-[1.02] ${role === 'admin'
