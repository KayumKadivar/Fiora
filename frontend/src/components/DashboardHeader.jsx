  ...
  return (
    <header className="h-16 bg-white/80 dark:bg-black/40 border-b border-zinc-200 dark:border-white/5 backdrop-blur-xl z-[100] px-4 sm:px-6 flex items-center justify-between shrink-0 shadow-sm">
      <div className="flex items-center space-x-3 sm:space-x-6">
        ...
        <h2 className="text-md sm:text-lg font-bold tracking-tight text-zinc-500 dark:text-zinc-400 truncate max-w-[150px] sm:max-w-none">
          {title}
        </h2>
      </div>
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className={`w-8 h-8 rounded-lg ${currentTheme.profileBg} flex items-center justify-center text-white font-black text-md shadow-md`}>
            ...
            <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl shadow-2xl p-4 backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex items-center space-x-3 mb-4 p-2 border-b border-zinc-100 dark:border-white/5 pb-4">
                ...
                <div className="overflow-hidden">
                  <h3 className="font-bold text-md truncate">{user?.name || 'User'}</h3>
                  <p className="text-[14px] text-zinc-500 truncate">{user?.email || 'user@fiora.com'}</p>
                </div>
              </div>
              <div className="space-y-1">
                <button className={`w-full flex items-center space-x-2 p-2.5 hover:bg-zinc-100 dark:hover:bg-white/5 rounded-xl transition-all text-zinc-600 dark:text-zinc-400 ${currentTheme.textHover} text-md font-medium`}>
                ...
                <button onClick={onLogout} className="w-full flex items-center space-x-2 p-2.5 hover:bg-rose-500/10 rounded-xl transition-all text-zinc-600 dark:text-zinc-400 hover:text-rose-500 text-md font-medium">
