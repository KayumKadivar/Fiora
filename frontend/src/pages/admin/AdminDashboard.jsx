      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-[2rem] p-6 shadow-sm backdrop-blur-sm h-full">
            <h4 className="text-[14px] font-black uppercase tracking-widest text-zinc-400 mb-4">Live Activity Monitor</h4>
              ...
              {stats.logs.length === 0 ? (
                ...
                stats.logs.map((log, idx) => (
                  <div key={idx} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors">
                    <div className="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center text-[14px] font-black text-blue-500">
                      {log.userName.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-black truncate">{log.userName}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-[2rem] p-8 shadow-sm backdrop-blur-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <button className="text-[14px] font-black uppercase tracking-widest text-blue-500">View All Logs</button>
          </div>
          <div className="space-y-4">
            {stats.logs.map((log, idx) => (
              <div key={idx} className="p-4 bg-zinc-50 dark:bg-black/20 border border-zinc-100 dark:border-white/5 rounded-2xl flex items-center justify-between group hover:border-blue-500/30 transition-all">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="font-black text-md">{log.contactName}</h4>
                    <p className="text-[14px] text-zinc-500 font-bold uppercase tracking-wider">Agent: {log.userName}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[14px] text-zinc-400 mt-1 font-bold">{new Date(log.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-zinc-50/50 dark:bg-emerald-500/5 border border-zinc-200 dark:border-emerald-500/10 rounded-[2rem] p-8 shadow-sm backdrop-blur-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-[14px] font-black text-zinc-400 uppercase tracking-widest">Global remidners</span>
          </div>
